"""
email_builder.py — Build the HTML weekly digest and send it via SendGrid.
"""
    ...
    <tr>
      <td style="padding:16px 36px;text-align:center;">
        <div style="color:#94a3b8;font-size:11px;line-height:1.7;">
          ...
          Powered by Claude AI &bull; SAM.gov &bull; Defense RSS feeds<br>
...
def send_email(html: str, subject: str | None = None) -> None:
    ...
    if not from_email:
        raise EnvironmentError("DIGEST_FROM_EMAIL is not set")
    if not to_raw:
    ...
    # SendGrid allows multiple personalizations; we send one per recipient
    # so that each To: line shows only the individual address
